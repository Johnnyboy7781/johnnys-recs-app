...
export class AppComponent implements OnInit, OnDestroy {
    ...
    constructor() {
        ...
        this.#breakpointObserver
            .observe([CUSTOM_BREAKPOINTS.LARGE, CUSTOM_BREAKPOINTS.SMALL])
            .pipe(takeUntil(this.destroyed))
            .subscribe((result) => {
                for (const query of Object.keys(result.breakpoints)) {
                    if (result.breakpoints[query]) {
                        this.recsService.screenSize.set(
                            query as CUSTOM_BREAKPOINTS
                        );
                    }
                }
            });
    ...
    ngOnInit(): void {
        this.recsService.sidenavSubject
            .pipe(tap(() => this.sidenav().toggle()))
            .subscribe();
    }
